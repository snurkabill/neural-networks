...
public class GeneralFunctionalityTest {
    ...
    public void learningFeedForward() {
        long seed = 0;
        ClassFullDatabase classFullDatabase = createDatabase(2000, false, seed);
        List<Integer> topology = Arrays.asList(classFullDatabase.getSizeOfVector(), 200, classFullDatabase.getNumberOfClasses());
    ...
    public void learningBinaryBinaryRBM() {
        ...
        ClassFullDatabase classFullDatabase = createDatabase(2000, true, seed);
        ...
        BinaryToBinaryRBM machine =
                new BinaryToBinaryRBM("RBM basicHeuristic",
                        (classFullDatabase.getSizeOfVector() + classFullDatabase.getNumberOfClasses()), 50,
                        new GaussianRndWeightsFactory(weightsScale, seed),
                        heuristic, seed);
        NetworkManager manager = new SupervisedRBMManager(machine, classFullDatabase, seed, null,
    ...
    private ClassFullDatabase createDatabase(int numOfElements, boolean binary, long seed) {
        ...
        try {
            ...
            return new ClassFullDatabase(seed, reader.getTrainingData(), reader.getTestingData(), "MNIST", true);
